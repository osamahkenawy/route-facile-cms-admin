...
const DaySurge = () => {
  ...
  const getCouponList = () => {
    ...
    const url = `${configWeb.GET_SURGE}?${params.toString()}`;
    // const url =configWeb.GET_SURGE
    // const url =`${configWeb.BASE_URL}admin/surge/14`
  ...
  const deleteDiscountCoupon = (id) => {
    return new Promise((resolve, reject) => {
      ...
      const url = configWeb.DELETE_SURGE(id);
  ...
  const handleEdit = (id) => {
    navigate(`/dynamic-pricing/edit-surge/${id}`);
  };
  ...
  return (
    <Container>
      <Row>
        ...
        >
          <Link to="/dynamic-pricing/create-surge">
            <Button className="btn-def">Add Surge</Button>
          </Link>
        </Col>
      </Row>
      <Col>
      ...
      <Modal show={showdel} onHide={handleCloseDel}>
        <Modal.Header closeButton>
          <Modal.Title>Delete Surge !</Modal.Title>
        </Modal.Header>
