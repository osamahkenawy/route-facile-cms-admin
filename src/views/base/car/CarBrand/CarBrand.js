...
const CarBrandList = () => {
  ...
  const getCarBrandList = () => {
    ...
    const url = `${configWeb.GET_BRANDS}?${params.toString()}`;
  ...
  const deleteCarBrand = (id) => {
    return new Promise((resolve, reject) => {
      ...
      const url = configWeb.DELETE_CAR_BRAND(id);
  ...
  const handleDelete = () => {
    deleteCarBrand(deleteID);
  };

  const handleEdit = (id) => {
    navigate(`/car/edit-brand/${id}`);
  };
                  ...
                  <td className="text-center">
                    <div className="car-listing-img-div-px">
                      <img
                        ...
                        src={`${imageFileServer}admin/car/brand/${item?.image}`}
