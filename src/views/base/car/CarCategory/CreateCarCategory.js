...
const CreateCarCategory = () => {
  ...
  const formSubmitFunction = ()=>{
    ...
    appendFormData.append("status", formData?.status)
      const url = id ? configWeb.PUT_CAR_CATEGORY_UPDATE(id) : configWeb.POST_CAR_CATEGORIES;
      setLoading(true);
  ...
  const getDetails = () => {
    return new Promise((resolve, reject) => {
      const url = configWeb.GET_CAR_CATEGORY_DETAILS(id);
  ...
  return (
      ...
      <Row>
        ...
        >
          <Link to="/car/category">
            <Button className="btn-def">Category List</Button>
          </Link>
        </Col>
      </Row>
    </div>
    {editLoading ? (
      ...
      <Row>
        ...
        <Col sm={12} md={6} lg={4} xl={3} className="mb-3">
          <Form.Group>
            <Form.Label>Category Name (Arabic)</Form.Label>
            <Form.Control
                  type="text"
                  name="name_ae"
          ...
          <div style={{ marginTop: '1rem' }}>
            ...
            src={
              formData.image instanceof File 
                ...
                : `${imageFileServer}admin/car/category/${formData.image}` // Server URL
            }
              alt="Preview"
