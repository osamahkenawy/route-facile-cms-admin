...
const NewDesignVote = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    const headerText = "New Design Vote Report";
    ...
    // Save the PDF
    const fileName = `New_Design_Vote_Report_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
  ...
  const handleDownloadExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "New Design Vote Report");

    // Generate Excel file and download
    const fileName = `New_Design_Vote_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
